...
Experimental change detectors are available in `src.utils.change_detection`. `CategoricalEDetector` implements mixtures of e-SR or e-CUSUM processes against a declared categorical null and reports an average-run-length bound. `ReferenceConditionalMonitor` uses a frozen reference, a DKW uncertainty band and a fixed mixture of corrected linear bets; its total error budget includes reference estimation failure. `PITMonitor` tests stability of sequentially ranked PIT values with histogram bets and a mixture over possible start times. It includes the unstarted mixture mass and reports post-alarm location separately. It can alarm when calibration improves. All three preserve event IDs and replay state; creating a fresh monitor requires a separately allocated budget where probability-of-alarm control is claimed. Their assumptions and paper adaptations are included in snapshots.
...
Measured campaigns and the paper-by-paper completion audit are in [the implementation plan](RESEARCH_IMPLEMENTATION_PLAN.md). The [core campaign](research/results/core-20260912.json) retains all 160 trials: the Dirichlet mixture detected more abrupt/gradual changes than KT in the tested scenarios, while the fixed-reference CTM variant had no detections for those alternatives. PITMonitor also detected improvements in calibration. Confidence regions covered each fixed true vector throughout the applicable trials; the conservative EV policy chose PASS throughout the uniform trials. These results have finite Monte Carlo uncertainty and do not establish a real-wheel advantage.

The [recalibration campaign](research/results/recalibration-20260912.json) found improvements from the three offline calibrators on synthetic overconfidence, but MCLLO overfit uniform data and the online variant worsened log loss in every tested scenario. Its finite-basis residual bound still held; that bound does not guarantee useful log loss. Four isotonic fits did not meet the numerical optimality-gap threshold despite solver success, which remains visible in their reports. The [representation campaign](research/results/representation-20260912.json) found no clear one-hot-versus-ordinal advantage across five trials per scenario. Recalibration and experimental detectors remain opt-in, and these measurements do not justify a larger model.
